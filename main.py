...
def visible_text(html):
    ...
    t = u" ".join(t.strip() for t in visibleText)
    # t = t.lower()
    t = word_tokenize(t)
    t = [w for w in t if w not in _stopwords]
    t = u' '.join(t)
...
def GenerateFileName(link):
    return "results/pages/" + ReplaceNonValidCharacters(link) + ".html"


def SaveWebResult(link, text):
    _Log(f'SaveWebResult("{link}", ...)')
    fname = GenerateFileName(link)
